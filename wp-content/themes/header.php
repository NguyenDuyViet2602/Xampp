...
<head>
	<title><?php if ( is_category() ) {
		echo __('Category Archive for &quot;', 'theme1662'); single_cat_title(); echo __('&quot; | ', 'theme1662'); bloginfo( 'name' );
	} elseif ( is_tag() ) {
		echo __('Tag Archive for &quot;', 'theme1662'); single_tag_title(); echo __('&quot; | ', 'theme1662'); bloginfo( 'name' );
	} elseif ( is_archive() ) {
		wp_title(''); echo __(' Archive | ', 'theme1662'); bloginfo( 'name' );
	} elseif ( is_search() ) {
		echo __('Search for &quot;', 'theme1662').wp_specialchars($s).__('&quot; | ', 'theme1662'); bloginfo( 'name' );
	} elseif ( is_home() || is_front_page()) {
		bloginfo( 'name' ); echo ' | '; bloginfo( 'description' );
	}  elseif ( is_404() ) {
		echo __('Error 404 Not Found | ', 'theme1662'); bloginfo( 'name' );
	} elseif ( is_single() ) {
		wp_title('');
	} else {
						...
						<?php if ( of_get_option('g_search_box_id') == 'yes') { ?>  
							<div id="top-search">
								<form method="get" action="<?php echo get_option('home'); ?>/">
									<input type="text" name="s"  class="input-search"/><input type="submit" value="<?php _e('GO', 'theme1662'); ?>" id="submit">
								</form>
							</div>  
						<?php } ?>
						<div class="clear"></div>
